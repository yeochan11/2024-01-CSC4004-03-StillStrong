...
def recipe_recommend():
    ...
    print(f'User_{user_id}?óêÍ≤? {result} Ï∂îÏ≤ú?ôÑÎ£?')
...
def provide_feedback():
    ...
    if feedback_counter < 100:
        ...
        print(f'feedback = {feedback} : ?óÖ?ç∞?ù¥?ä∏ ?ôÑÎ£?')
    else:
...
def saveModel():
    ...
    print("????û• ?õÑ ?îå?ùº?ä§?ä∏ ?ÑúÎ≤? Ï¢ÖÎ£å.")
